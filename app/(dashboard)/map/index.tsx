...
const fallbackMarkers: Place[] = [
  { id: "1", name: "Demo Vet Clinic A", latitude: 6.928, longitude: 79.86 },
  { id: "2", name: "Demo Pet Shop B", latitude: 6.93, longitude: 79.865 },
];
...
const MapScreen = () => {
  ...
  const fetchPlaces = async (lat: number, lng: number) => {
    try {
      ...
      const RADIUS_STEPS = [3000, 6000, 9000, 10000]; // max 10km
      const MIN_RESULTS = 5;

      const collected: Place[] = [];
      for (let i = 0; i < RADIUS_STEPS.length; i++) {
        const radius = RADIUS_STEPS[i];
        const url = `https://overpass-api.de/api/interpreter?data=[out:json][timeout:60];
(
  nwr["amenity"="veterinary"](around:${radius},${lat},${lng});
  nwr["healthcare"="veterinary"](around:${radius},${lat},${lng});
  nwr["office"~"veterin"](around:${radius},${lat},${lng});
  nwr["amenity"="clinic"](around:${radius},${lat},${lng});
  nwr["amenity"="hospital"](around:${radius},${lat},${lng});
  nwr["shop"="pet"](around:${radius},${lat},${lng});
  nwr["shop"="pet_grooming"](around:${radius},${lat},${lng});
  nwr["amenity"="animal_shelter"](around:${radius},${lat},${lng});
  nwr["amenity"="kennel"](around:${radius},${lat},${lng});
  nwr["amenity"="animal_boarding"](around:${radius},${lat},${lng});
  nwr["amenity"="animal_breeding"](around:${radius},${lat},${lng});
  nwr["shop"="animal"](around:${radius},${lat},${lng});
);
out geom;`;
        ...
        if (data.elements && data.elements.length > 0) {
          ...
          try {
            ...
            console.warn("Error mapping Overpass response", err);
          }

          // merge & deduplicate
          const map = new Map<string, Place>();
          [...collected, ...batch].forEach((pl) => map.set(pl.id, pl));
...
places.forEach(function(p){
        ...
        {region && (
          <>
            ...
            <View style={styles.debugBox}>
              <Text style={styles.debugTitle}>Nearby Pet Care (OSM)</Text>
              {loadingPlaces && (
  ...
  return (
    <View style={styles.container}>
      ...
      {region && (
        <View style={styles.debugBox}>
          <Text style={styles.debugTitle}>Nearby Pet Care</Text>
          ...
          {!loadingPlaces && showFallback && (
            <Text style={styles.debugLine}>
              {errorMsg
                ? errorMsg
                : apiStatus === "NO_RESULTS"
                  ? "No pet care places found (demo markers)."
                  : apiStatus
                    ? `Status: ${apiStatus}`
                    : "No data."}
            </Text>
